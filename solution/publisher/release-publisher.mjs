...
const KEY_FILE = process.env.SIGNING_KEY_PATH || '/app/keys/current/current.key.pem';
const CERT_FILE = process.env.SIGNING_CERT_PATH || '/app/keys/current/current.cert.pem';

// duckdb uses callbacks, promisify the two I need
...
async function getPublishableBundles(db) {
  ...
  await exec(db, `CREATE OR REPLACE TABLE manifest AS
...
async function main() {
  ...
  await exec(db, `CREATE TABLE IF NOT EXISTS publications (
  ...
  for (const b of bundles) {
    ...
    if (prev.length) {
      ...
      await exec(db, `INSERT INTO publications VALUES (?,?,?,?,?,?,?,?)`,
        b.bundle_id, receipt.request_token, receipt.publication_id, receipt.status,
        key.key_id, b.artifact_count, b.total_bytes, descriptor);
    }
